...
export class Analyze<
  ...
  const Parser extends Analyze<any, any, any> | undefined = undefined
>{
  readonly input: Path
  ...
  constructor(input: Path, base?: Parser, ast?: AST<Path>) {
    this.input = input
    this.errors = []
    
    this.base = base
    this.ast = ast ?? new AST(this.input, this.errors)
  }
   ...
   * 3. Is Parser Flag (1 byte): 1 if this instance is a parser, 0 otherwise.
   * 4. Has Base Flag (1 byte): 1 if a base parser exists, 0 otherwise.
   * 5. Base Length (optional, 4 bytes, LE): Byte size of the serialized base parser.
   * 6. Base Data (optional, variable): Recursively serialized base parser data.
   * 
   ...
   * @returns {Analyze<string, any, any>} A reconstructed Analyze instance.
   */
  static fromBuffer(buffer: Buffer): Analyze<string, any, any> {
    let cursor = 0
    ...
    let base: Analyze<string, any, any> | undefined
    if (hasBase) {
    ...
    return new Analyze(ast.input, base, ast)
  }
}
