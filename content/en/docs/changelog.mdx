...
Full change history for url-ast.
...
- Build process migrated to **esbuild** (CJS/ESM), targeting **ES2024**, with an auxiliary bundle for `decodeURL`.
...
- **README.md** rewritten for v3: `.type` / `=` / `~`, colon and bracket routes, catch-all `[...]`, query and fragment behavior with and without a base, `Analyze.create` / `fromBuffer` / `getBuffer`, AST `display` / `toJSON`, public exports table, Node 20 + TypeScript ^5 requirements.
