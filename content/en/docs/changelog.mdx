...
- Renamed the `display()` method to `toString()` in the `AST` and `ErrorLog` classes, adopting a language-native string-conversion standard.
...
- **README.md** rewritten for v3: `.type` / `=` / `~`, routes with `:` and `[]`, catch-all `[...]`, query and fragment behavior with/without base, `Analyze.create` / `fromBuffer` / `getBuffer`, AST `toString` / `toJSON`, public exports table, Node 20 + TypeScript ^5 requirements.
